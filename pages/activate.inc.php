...
if(isset($_GET['c']) && isset($_GET['u'])) {
	...
	$MySQL['query'] = "SELECT `id` FROM `users` WHERE `activationcode` = '".$_GET['c']."' AND `activated` = '0' AND `id` = '".$_GET['u']."' LIMIT 1";
	$MySQL['result'] = $MySQL['connection']->query($MySQL['query']);
	if($MySQL['result']->num_rows==1){
		$MySQL['query'] = "UPDATE `users` SET `activated` = '1', `activationcode` = '0' WHERE  `id` = '".$_GET['u']."'";
		$MySQL['result'] = $MySQL['connection']->query($MySQL['query']);
